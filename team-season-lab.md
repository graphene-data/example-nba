...
select
  ...
  team_name_away as opponent,
  'Home' as location,
  game_date,
  wl_home as wl,
  pts_home as for_points,
  pts_away as against_points,
  fgm_home as fgm,
  fga_home as fga,
  fg3m_home as fg3m,
  fg3a_home as fg3a,
  ...
  fgm_away as opp_fgm,
  fga_away as opp_fga,
  fg3m_away as opp_fg3m,
  fg3a_away as opp_fg3a,
  ftm_away as opp_ftm,
  fta_away as opp_fta,
  oreb_away as opp_oreb,
  dreb_away as opp_dreb,
  reb_away as opp_reb,
  ast_away as opp_ast,
  tov_away as opp_tov
from game
...
<Row>
  ...
  <Dropdown title="Season" name="selected_season" data="season_options" value="season_key" label="season" defaultValue="S2022" />
</Row>
...
where team = $selected_team
  and season_key = $selected_season
```
...
select
  ...
  round(three_point_rate, 1) as three_point_rate
from league_team_seasons
where season_key = $selected_season
```
...
<Row>
  <ScatterPlot
    title="Record vs scoring margin"
    data="league_context"
    x="net_points"
    y="win_pct"
    splitBy="team_group"
  ...
  />
  <ScatterPlot
    title="Shot profile: threes vs free throws"
    data="league_context"
    x="three_point_rate"
    y="free_throw_rate"
    splitBy="team_group"
...
<BarChart
  title="Style and four-factor profile vs league average"
  data="four_factors"
  x="metric"
  y="selected_value, league_average"
  arrange="group"
  label
  height="380px"
/>
...
from team_games
where team = $selected_team
  and 'S' || season = $selected_season
order by game_date
```

<Row>
  <LineChart
    title="Cumulative scoring margin"
    data="game_log"
    x="game_number"
    y="cumulative_margin"
    height="340px"
  />
  <ScatterPlot
    title="Game-level shot quality and margin"
    data="game_log"
    x="efg_pct"
    y="margin"
    splitBy="location"
...
<Row>
  ...
  />
  <BarChart
    title="Home/away scoring margin"
    data="home_away_split"
    x="location"
    y="net_points"
    label
